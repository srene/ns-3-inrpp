 ...
 */
// Scenario 1 dumbbell topology with INRPP and TCP flows mixed
// Network topology
/*
			 n6				   n1
			  \			  1.2 /   \ 2.1
...
//	m_rv_flow_intval->SetAttribute("Stream", IntegerValue(2));
	...
	for(uint32_t i=0;i<n;i++)
	{
		...
		if(i>4)Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TcpNewReno::GetTypeId ()));
...
void LogFairness(){
	...
	if(sink.size()>0)Simulator::Schedule(Seconds(0.1),&LogFairness);
