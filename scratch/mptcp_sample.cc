...
void StopFlow(Ptr<MpTcpPacketSink> p, uint16_t port,uint32_t size);
...
main (int argc, char *argv[])
{
	...
	Config::SetDefault("ns3::MpTcpSocketBase::MaxSubflows", UintegerValue(100)); // Sink
	Config::SetDefault("ns3::MpTcpSocketBase::CongestionControl", StringValue("COUPLED_INC"));
   ...
   //Read RocketFuel Topology
    ...
    //for(uint32_t pair = 0; pair < num_customers; pair++)
	{
	   ...
	   for(uint32_t i=0;i<n;i++)
			...
			NetDeviceContainer destLink,destLink2;
			...
			uint32_t dst2Link = x->GetInteger();
			//while(dst2Link==dest||dst2Link==pair) dst2Link = x->GetInteger();
			destLink2 = pointToPoint.Install (CustomerRouters.Get(dst2Link), receivers.Get(num_pair));

			std::stringstream netAddr3;
			netAddr3 << dest_net2 << "." << net << "." << (num) << ".0";
			str = netAddr3.str();
			ipv4.SetBase(str.c_str(), "255.255.255.0");
			Ipv4InterfaceContainer iDest2 = ipv4.Assign (destLink2);
			...
			MpTcpBulkSendHelper source("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address(iDest.GetAddress (1)), port));
			...
			Ptr<MpTcpBulkSendApplication> bulk = DynamicCast<MpTcpBulkSendApplication> (sourceApps.Get (0));
			bulk->SetCallback(MakeCallback(&StartLog));
			bulk->SetNetDevice(sourceLink.Get(1));
			Ptr<MpTcpPacketSink> psink = DynamicCast<MpTcpPacketSink> (sinkApps.Get (0));
