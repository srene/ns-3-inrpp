...
void StartLog(Ptr<Socket> socket,Ptr<NetDevice> net);
...
//
  ...
  devices7 = pointToPoint.Install (nodes.Get(7),nodes.Get(1));
  devices8 = pointToPoint.Install (nodes.Get(9),nodes.Get(8));
...
//
  ...
  Ipv4InterfaceContainer i7 = ipv4.Assign (devices7);
  ipv4.SetBase ("10.0.8.0", "255.255.255.0");
  Ipv4InterfaceContainer i8 = ipv4.Assign (devices8);
  ipv4.SetBase ("10.0.9.0", "255.255.255.0");
  Ipv4InterfaceContainer i9 = ipv4.Assign (devices9);
  ipv4.SetBase ("10.0.10.0", "255.255.255.0");
  Ipv4InterfaceContainer i10 = ipv4.Assign (devices10);
//
...
/*Configure detour path at n0 SPFGetNextLink
  ...
  PointerValue ptr;
  devices0.Get(1)->GetAttribute ("TxQueue", ptr);
  ...
  PointerValue ptr12;
  devices1.Get(0)->GetAttribute ("TxQueue", ptr12);
  ...
  PointerValue ptr13;
  devices7.Get(1)->GetAttribute ("TxQueue", ptr13);
  Ptr<Queue> txQueue13 = ptr13.Get<Queue> ();
  std::ostringstream osstr13;
  osstr13 << "netdevice_9.bf";
  Ptr<OutputStreamWrapper> streamtr13 = asciiTraceHelper.CreateFileStream (osstr13.str());
  txQueue13->GetObject<DropTailQueue>()->TraceConnectWithoutContext ("BytesQueue", MakeBoundCallback (&BufferChange, streamtr13));

  txQueue2->TraceConnectWithoutContext ("Drop", MakeCallback (&Drop));

  txQueue12->TraceConnectWithoutContext ("Drop", MakeCallback (&Drop));

  txQueue13->TraceConnectWithoutContext ("Drop", MakeCallback (&Drop));
