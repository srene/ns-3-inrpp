...
//
  ...
  devices6 = pointToPoint.Install (nodes.Get(3),nodes.Get(6));


  pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("0.1Gbps"));
  ...
  //
   ...
   uint16_t port = 9100;  // well-known echo port number

 	  BulkSendHelper source ("ns3::TcpSocketFactory",
 	                         InetSocketAddress (i6.GetAddress (1), port));
 	  ...
 	  ApplicationContainer sinkApps = sink1.Install (nodes.Get(6));
 	  ...
 	  bulk->SetNetDevice(devices5.Get(0));
   ...
   NodeContainer receivers;
	  ...
	  if(tracing2)
	  {
			...
			PointerValue ptr3;
			devices6.Get(0)->GetAttribute ("TxQueue", ptr3);
			Ptr<Queue> txQueue3 = ptr3.Get<Queue> ();
			std::ostringstream osstr3;
			osstr3 << folder << "/netdevice_" <<n+1<<".bf";
			...
			osstr << folder << "/netdeviceRx_"<<n+1<<".tr";
  ...
  for(uint32_t i=0;i<n;i++)
  {
	  ...
	  pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("1Gbps"));
	  NetDeviceContainer sourceLink = pointToPoint.Install (nodes.Get(7+i),nodes.Get(4));
	 // pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("1Gbps"));

	  NetDeviceContainer destLink = pointToPoint.Install (nodes.Get(3),nodes.Get(7+n+i));

	  InternetStackHelper internet;
	  internet.Install (nodes.Get(7+i));
	  internet.Install (nodes.Get(7+n+i));

	  senders.Add(nodes.Get(7+i));
	  receivers.Add(nodes.Get(7+n+i));
	  ...
	  if(tracing2)
	  {
	  	...
	  	Ptr<Queue> txQueue3 = ptr3.Get<Queue> ();
	  	std::ostringstream osstr3;
	  	osstr3 << folder << "/netdevice_" <<7+i<<".bf";
	  	Ptr<OutputStreamWrapper> streamtr3 = asciiTraceHelper.CreateFileStream (osstr3.str());
	  	txQueue3->GetObject<DropTailQueue>()->TraceConnectWithoutContext ("BytesQueue", MakeBoundCallback (&BufferChange, streamtr3));
	  }
	  ...
	  // Set the amount of data to send in bytes.  Zero is unlimited.
	  source.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
	  ApplicationContainer sourceApps = source.Install (nodes.Get(7+i));
	  sourceApps.Start (Seconds (1.0+(i*time)));
	  sourceApps.Stop (Seconds (stop));

	  PacketSinkHelper sink1 ("ns3::TcpSocketFactory",
						   InetSocketAddress (Ipv4Address::GetAny (), port));
	  ApplicationContainer sinkApps = sink1.Install (nodes.Get(7+n+i));
  ...
  if (tracing)
	{
	  ...
	  //pointToPoint.EnablePcap(osstr.str(),devices5.Get(0), false);
	  //pointToPoint.EnablePcap(osstr.str(),devices6.Get(1), false);
