...
#include "brite-inrpp-topology-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("BriteInrppTopologyHelper");

BriteInrppTopologyHelper::BriteInrppTopologyHelper (std::string confFile,
                                          std::string seedFile,
...
BriteInrppTopologyHelper::~BriteInrppTopologyHelper ()
{
  NS_LOG_FUNCTION (this);
  delete m_topology;
...
void
BriteInrppTopologyHelper::AssignStreams (int64_t streamNumber)
{
  m_uv->SetStream (streamNumber);
}

void
BriteInrppTopologyHelper::BuildBriteNodeInfoList (void)
{
  NS_LOG_FUNCTION (this);
...
void
BriteInrppTopologyHelper::BuildBriteEdgeInfoList (void)
{
  NS_LOG_FUNCTION (this);
  brite::Graph *g = m_topology->GetGraph ();
...
Ptr<Node>
BriteInrppTopologyHelper::GetLeafNodeForAs (uint32_t asNum, uint32_t leafNum)
{
  return m_asLeafNodes[asNum]->Get (leafNum);
}

Ptr<Node>
BriteInrppTopologyHelper::GetNodeForAs (uint32_t asNum, uint32_t nodeNum)
{
  return m_nodesByAs[asNum]->Get (nodeNum);
}

uint32_t
BriteInrppTopologyHelper::GetNNodesForAs (uint32_t asNum)
{
  return m_nodesByAs[asNum]->GetN ();
}

uint32_t
BriteInrppTopologyHelper::GetNLeafNodesForAs (uint32_t asNum)
{
  return m_asLeafNodes[asNum]->GetN ();
}

uint32_t
BriteInrppTopologyHelper::GetNNodesTopology () const
{
  return m_numNodes;
}

uint32_t
BriteInrppTopologyHelper::GetNEdgesTopology () const
{
  return m_numEdges;
}

uint32_t
BriteInrppTopologyHelper::GetNAs (void) const
{
  return m_numAs;
}

uint32_t
BriteInrppTopologyHelper::GetSystemNumberForAs (uint32_t asNum) const
{
  return m_systemForAs[asNum];
}

void BriteInrppTopologyHelper::GenerateBriteTopology (void)
{
...
void
BriteInrppTopologyHelper::BuildBriteTopology (InrppStackHelper& stack)
{
  NS_LOG_FUNCTION (this);
...
void
BriteInrppTopologyHelper::BuildBriteTopology (InrppStackHelper& stack, const uint32_t systemCount)
{
  NS_LOG_FUNCTION (this);
  ...
  //create nodes
  for (BriteInrppTopologyHelper::BriteNodeInfoList::iterator it = m_briteNodeInfoList.begin (); it != m_briteNodeInfoList.end (); ++it)
    {
...
void
BriteInrppTopologyHelper::AssignIpv4Addresses (Ipv4AddressHelper& address)
{
  NS_LOG_FUNCTION (this);
  //assign IPs
...
void
BriteInrppTopologyHelper::AssignIpv6Addresses (Ipv6AddressHelper& address)
{
  NS_LOG_FUNCTION (this);
...
void
BriteInrppTopologyHelper::ConstructTopology ()
{
  NS_LOG_FUNCTION (this);
  ...
  for (BriteInrppTopologyHelper::BriteEdgeInfoList::iterator it = m_briteEdgeInfoList.begin (); it != m_briteEdgeInfoList.end (); ++it)
    {
      // Set the link delay
      ...
      NS_LOG_LOGIC("Datarate " << DataRate ((*it).bandwidth * mbpsToBps));
      m_britePointToPointHelper.SetDeviceAttribute ("DataRate",
                                                    DataRateValue (DataRate ((*it).bandwidth * mbpsToBps)));
      m_britePointToPointHelper.SetQueue ("ns3::InrppTailQueue",
                               "LowerThBytes", UintegerValue (2500*1500),
                               "HigherThBytes", UintegerValue (4000*1500),
    						   "MaxBytes", UintegerValue(5000*1500));
